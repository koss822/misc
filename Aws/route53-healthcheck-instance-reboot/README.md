...
```bash
route53-healthcheck-instance-reboot$ sam build --use-container
```
...
```bash
route53-healthcheck-instance-reboot$ sam local invoke HelloWorldFunction --event events/event.json
```

The SAM CLI can also emulate your application's API. Use the `sam local start-api` to run the API locally on port 3000.

```bash
route53-healthcheck-instance-reboot$ sam local start-api
route53-healthcheck-instance-reboot$ curl http://localhost:3000/
```
...
```bash
route53-healthcheck-instance-reboot$ sam logs -n HelloWorldFunction --stack-name route53-healthcheck-instance-reboot --tail
```
...
```bash
route53-healthcheck-instance-reboot$ pip install pytest pytest-mock --user
route53-healthcheck-instance-reboot$ python -m pytest tests/ -v
```
